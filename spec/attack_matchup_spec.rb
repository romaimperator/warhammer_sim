...
#  subject { AttackMatchupFactory.new }
  ...
  describe "#attack" do
    it "returns a matchup result object" do
      assert_instance_of AttackMatchupResult, AttackMatchupFactory().attack
    end

    it "records results which make sense" do
      attack_count = 5
      matchup = AttackMatchupFactory(attack: Attack.new(attack_count, 3, 3, []))
    ...
    [:roll_hits,
     ...
     :wound_reroll_values,
      ...
      it "calls the #{callback} method on equipment" do
        ...
        AttackMatchupFactory(attack: Attack.new(5, 3, 3, [item])).attack
        expect(item).to have_received(callback).at_least(:once)
      end
    end

    it "skips the rolling phase if the weapon_skill is :auto_hit" do
      result = AttackMatchupFactory(attack: Attack.new(5, 3, 3, [Equipment::AutoHit.new])).attack
  ...
  describe "#compute_hits" do
    it "computes hits normally" do
      ...
      matchup = AttackMatchupFactory(attack: Attack.new(4, 3, 3, []))
    ...
    it "returns hits as same as attacks when the weapon skill is :auto_hit" do
      attack_count = 4
      matchup = AttackMatchupFactory(attack: Attack.new(attack_count, 3, 3, [Equipment::AutoHit.new]))
  ...
  describe "#compute_wounds" do
    it "computes wounds normally" do
      ...
      AttackMatchupFactory().compute_wounds(4)
      expect(ComputeWounds).to have_received(:new)
    end

    it "returns wounds as same as hits when the attack includes AutoWound" do
      hits = 4
      matchup = AttackMatchupFactory(attack: Attack.new(1, 3, 3, [Equipment::AutoWound.new]))
